...
public class StolicLineGraphicView extends View{
     ...
     * 2纵坐标值
     */
    private ArrayList<Double> zRawData;
    /**
     * 横坐标值
     */
    ...
    public StolicLineGraphicView(Context context)
    {
        this(context, null);
    }

    public StolicLineGraphicView(Context context, AttributeSet attrs)
    {
        super(context, attrs);
...
//            }
        ...
        mPaint.setColor(res.getColor(R.color.color_640085));
        for (int i = 0; i < zPoints.length; i++)
        {
//            if(yRawData.get(i)!=0.0){
            canvas.drawCircle(zPoints[i].x, zPoints[i].y, 12 / 2, mPaint);
//            }
        }
    }
    ...
    private void drawAllXLine(Canvas canvas)
    {
        for (int i = 0; i < spacingHeight + 1; i++)
        {
            ...
            drawText((i*averageValue)+"", 2, bheight - (bheight / spacingHeight) * i + marginTop,
                    canvas);
        }
    }
...
//                Color.argb(10, 0, 255, 255) }, null, Shader.TileMode.CLAMP);
        Shader mShader = new LinearGradient(0, 0, 0, getHeight(), new int[] {
                Color.argb(100, 100, 0, 133), Color.argb(90, 100, 0, 133),
                Color.argb(15,100, 0, 133) ,Color.argb(0,100, 0, 133)}, null, Shader.TileMode.CLAMP);
        framPanint.setShader(mShader);
        Path path = new Path();
...
//            if(yRawData.get(i) != 0.0){
            canvas.drawLine(startp.x, startp.y, endp.x, endp.y, mPaint);
//            }
            if(i == zPoints.length - 2){
                path.lineTo(endp.x, endp.y);
                path.lineTo(endp.x, getHeight()-juHeght);
                path.lineTo(zPoints[0].x,getHeight()-juHeght);
                path.lineTo(zPoints[0].x,zPoints[0].y);
                path.close();
                canvas.drawPath(path, framPanint);
            }
        }
    ...
    private Point[] getzPoints()
    {
        Point[] points = new Point[zRawData.size()];
        for (int i = 0; i < zRawData.size(); i++)
        {
            int ph = bheight - (int) (bheight * (zRawData.get(i) / maxValue));
            points[i] = new Point(xList.get(i), ph + marginTop);
        }
        return points;
    }

    public void setData(ArrayList<Double> yRawData,ArrayList<Double> zRawData, int maxValue, int averageValue)
    {
        ...
        this.zPoints = new Point[zRawData.size()];
        this.xRawDatas = xRawDatas;
        this.yRawData = yRawData;
        this.zRawData = zRawData;
